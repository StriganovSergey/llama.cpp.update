#!/bin/bash
# =============================================================================
# Unified Update Script v4.7 — DFD Compliant Edition (Fixed Flow Control)
...
#          FIXES: Strict error propagation, verification steps, atomic rollback.
# =============================================================================
# <PLAN>
...
# 9. Deployment Verification & Health Check
# 10. Main orchestration with strict error handling
#
# Risks mitigated: broken package state, CUDA compiler discovery,
# deprecated GPU targets, service state loss, mixed GPU handling,
# partial failures (build fail -> deploy).
# </PLAN>
...
DEBUG_MODE=false   # или false по умолчанию
while [[ $# -gt 0 ]]; do
    case "$1" in
...
deploy_binaries() {
    ...
    if [ ! -d "$src_dir" ]; then
        log_error "Build directory not found: $src_dir"
        return 1
    fi

    log_info "Deploying binaries from $src_dir to $dst_dir"
    if cp -f "${src_dir}/llama-"* "${dst_dir}/" 2>/dev/null; then
...
main() {
   ...
   check_all_requirements || { log_error "Requirements check failed."; exit 1; }
    ...
    case "$action" in
        1)
            ...
            # 1. Stop Service
            stop_service || { log_error "Failed to stop service"; exit 1; }

            # 2. Create Backup (Critical for rollback)
            create_backup || { log_error "Failed to create backup"; exit 1; }
            ...
            if ! start_service; then
                ...
                # If service fails to start, it might be config related, but we rollback repo to be safe
                handle_update_failure
                exit 1
            fi
